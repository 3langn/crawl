      ...
      request(options, function (err, response, html) {
        ...
        if (listProperties && itemProperties) {
          ...
          if (listProperties.childElement || listProperties.childClass) {
            ...
            var itemList = cheerio.load(childElement)(
              listProperties.childElement
                ? listProperties.childElement
                : listProperties.childClass
            );
          }

          let urlList = [];
          for (let i = 0; i < itemList.length; i++) {
            const itemUrl = cheerio
              .load(page(itemList[i]).html())(itemProperties.element)
              .first()
              .prop(itemProperties.elementProp);
