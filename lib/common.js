...
function func_repairPhone(text) {
  var target = text;
...
function func_extractPrice(priceString) {
  ...
  if (priceString.indexOf("Thỏa thuận") > -1) {
    ...
    priceString.indexOf("tỷ") > -1 &&
    priceString.indexOf("triệu") > -1
  ) {
    var tyPrice = priceString.split("tỷ")[0].replace("tỷ", "").trim();
    var trieuPrice = priceString
      .split("tỷ")[1]
      .replace("triệu", "")
      .replace("tỷ", "")
      .trim();
    ...
    outputPrice = (
      priceString.replace("Tỷ", "").trim() * 1000000000
    ).toString();
  } else if (priceString.indexOf("tỷ") > -1) {
    outputPrice = (
      priceString.replace("tỷ", "").trim() * 1000000000
    ...
    outputPrice = (
      priceString.replace("triệu / m2", "").trim() * 1000000
    ).toString();
  } else if (priceString.indexOf("Triệu/m2") > -1) {
    outputPrice = (
      priceString.replace("Triệu/m2", "").trim() * 1000000
    ...
    outputPrice = (
      priceString.replace("triệu", "").trim() * 1000000
    ).toString();
  }
  return outputPrice;
}
...
function func_validateOutput(output) {
  ...
  itemData.sodienthoai = func_repairPhone(itemData.sodienthoai);
